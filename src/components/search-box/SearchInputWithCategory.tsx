...
import CategoryMenuItem from "@component/categories/CategoryMenuItem";
...
import StyledNavbar from "@component/navbar/styles";
...
type SearchInputWithCategoryProps = {
  serviceList?: Service[];
};
...
const SearchInputWithCategory: FC<SearchInputWithCategoryProps> = ({
  serviceList = [],
}) => {
  ...
  const renderNestedNav = (list: any[], isRoot = false) => {
    return list?.map((nav: any) => {
      if (isRoot) {
        if (nav.parentCategoryId != null && nav.isStoreVisible) {
          return (
            <FlexBox flexDirection={"column"}>
              <NavLink
                href={"/catalog/products/?category=" + nav.id}
              ...
              >
                <CategoryMenuItem
                  ...
                  mediaPath={renderImageCategoryImageSource(
                    nav?.categoryImages
                  )}
                  caret={nav?.subCategories?.length > 0 ? true : false}
                ></CategoryMenuItem>
                <Box className="child" pl="0.5rem">
                  <Card borderRadius={8} minWidth="230px">
                    {renderNestedNav(
                      nav.subCategories,
                      nav.parentCategoryId == null ? true : false
                    )}
                  </Card>
                </Box>
              </NavLink>
          ...
          return (
            ...
            >
              <CategoryMenuItem
                ...
                // icon={item.icon}
                title={nav.categoryName}
                mediaPath={renderImageCategoryImageSource(nav?.categoryImages)}
                caret={nav?.subCategories?.length > 0 ? true : false}
              ></CategoryMenuItem>
        ...
        if (
          ...
          return (
            ...
            >
              ...
              <Box className="child" pl="0.5rem">
                <Card borderRadius={8} boxShadow="large" minWidth="230px">
                  {renderNestedNav(
                    nav.subCategories,
                    nav.parentCategoryId == null ? true : false
                  )}
          ...
          if (nav.isStoreVisible && nav.parentCategoryId == null) {
            return (
              <Box
                className="parent"
                position="relative"
  ...
  return (
    <>
      <FlexBox justifyContent={"center"} alignItems="center" mr="1rem">
        <Menu
          direction="left"
          className="category-dropdown"
          handler={
            <StyledFlexBox className="dropdown-handler" alignItems="center">
              <Typography color="#672DD1" fontWeight="500" fontSize="1rem">
                <span className={quicksand.className}>Categories</span>
              </Typography>
        ...
        >
          ...
          {/* {serviceList.map((item) => {
            return (
              <>
                {item.isStoreVisible && item?.parentCategoryId == null && (
                  <CategoryMenuItem
                    // key={item.title}
                    ...
                    caret={item?.subCategories?.length > 0 ? true : false}
                  >
                    {item?.subCategories?.length > 0 && (
                      <>
                        {item?.subCategories.map((subCat) => {
                          <CategoryMenuItem
                            // key={item.title}
                            href={"/catalog/products/?category=" + subCat.id}
                            // icon={item.icon}
                            title={subCat.categoryName}
                            mediaPath={renderImageCategoryImageSource(
                              subCat?.categoryImages
                            )}
                            caret={
                              subCat?.subCategories?.length > 0 ? true : false
                            }
                          ></CategoryMenuItem>;
                        })}
                      </>
